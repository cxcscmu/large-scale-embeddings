...
class ClueWeb22Searcher:
    ...
    def search(self, query_text, k=10, num_of_shards=4, with_distance=False):
        """
        ...
        """
        raw_docs = list()
        qid = self._encode_query_text(query_text)

        # Step 1: Iterate through each shard and collect results
...
def test_time_performance(top_k):
    """
    ...
    for i in range(100):
        ...
        docids = searcher.search(random_query, k=top_k)
    ...
    print(f"  Total time for 100 searching operations: {total_time:.4f} seconds")
...
def run_trec_evaluation():
    ...
    queries_path = "data/trec_eval/queries_dev_cleaned.tsv"
    with open(queries_path, 'r', encoding='utf-8') as f:
        ...
        for row in tqdm(reader):
            ...
            docids = searcher.search(query_text, k=top_k)
            doc_texts = searcher.get_doc_texts(docids)
            retrieved_dict[qid] = doc_texts
    
    # Evaluate using MRR@k metric
    r_metric = RMetric(qrels_path="data/trec_eval/qrels_dev_cleaned.tsv")
    r_metric.mrr_at_k(retrieved_dict, k=top_k, verbose=False)
